...
class SendMessageLogicTest {
	...
	void sendCloseMessageToAllApplicableAffecteds() {
		...
		assertThat(messageRequest.getMessages()).containsExactly(
			new Message()
				...
				.party(new Party().partyId("partyId-2"))
			...
			new Message()
				...
				.headers(List.of(
					...
					new Header().name(NameEnum.CATEGORY).values(List.of(String.valueOf(CATEGORY)))))
				.party(new Party().partyId("partyId-4"))
			...
			new Message()
				...
				.headers(List.of(
					...
					new Header().name(NameEnum.CATEGORY).values(List.of(String.valueOf(CATEGORY)))))
				.party(new Party().partyId("partyId-6"))
		...
		assertThat(disturbanceFeedbackHistoryEntityCaptor.getAllValues())
			...
			.containsExactly(
				tuple(CATEGORY.toString(), DISTURBANCE_ID, "partyId-2"),
				tuple(CATEGORY.toString(), DISTURBANCE_ID, "partyId-4"),
				tuple(CATEGORY.toString(), DISTURBANCE_ID, "partyId-6"));
	}
	...
	void sendCloseMessageToProvidedApplicableAffecteds() {
		...
		affectedEntity3.setPartyId("partyId-4"); // Same as affectedEntity1 in order to test that we can have the same partyId on another AffectedEntity.
		...
		final var affectedEntitiesOverride = new ArrayList<AffectedEntity>(List.of(affectedEntity1, affectedEntity2, affectedEntity3));
	...
	void sendUpdateMessageWherePlannedStartAndStopDatesAreNotSet() {
		...
		assertThat(messageRequest.getMessages()).containsExactly(
			new Message()
				...
				.headers(List.of(
					...
					new Header().name(NameEnum.CATEGORY).values(List.of(String.valueOf(CATEGORY)))))
				.party(new Party().partyId("partyId-1"))
		...
		assertThat(disturbanceFeedbackHistoryEntityCaptor.getAllValues())
			.extracting(DisturbanceFeedbackHistoryEntity::getCategory, DisturbanceFeedbackHistoryEntity::getDisturbanceId, DisturbanceFeedbackHistoryEntity::getPartyId)
			.containsExactly(
				tuple(CATEGORY.toString(), DISTURBANCE_ID, "partyId-1"),
				tuple(CATEGORY.toString(), DISTURBANCE_ID, "partyId-2"));
	}
